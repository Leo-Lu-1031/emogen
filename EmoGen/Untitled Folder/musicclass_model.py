...
class PerformanceRNN(nn.Module):
    def __init__(
        ...
        self.primary_event = list(torch.nn.functional.one_hot(
            torch.tensor(self.event_dim - 1), self.event_dim))
        #print(self.primary_event)
        ...
        self.output_fc = nn.Sequential(
            nn.Linear(gru_layers*self.input_dim, 512),
            nn.ReLU(),
            nn.Linear(512, 64),
            nn.ReLU(),
    ...
    def forward(self, event, control=None, hidden=None):
        ...
        #output = output.reshape([1,batch_size,self.attention_dim*self.head_dim])
        #output = torch.matmul(self.attention_activation(torch.matmul(Q,torch.transpose(K,1,2)))/math.sqrt(self.attention_dim), V)
        #print(output.shape)
    ...
    def generate(
        ...
        hidden = torch.tensor(np.zeros([self.gru_layers, batch_size, self.hidden_dim])).to(device).float()
        ...
        for step in step_iter:
            ...
            #print(event.shape)
            output, hidden = self.forward(event, hidden=hidden)
            ...
            if use_teacher_forcing and step < steps - 1:  # avoid last one
                if True:#np.random.random() <= teacher_forcing_ratio:
                    event = events[step].unsqueeze(0)
                    print('e', event.shape)

        return outputs[-1]

    def beam_search(self, init, steps, beam_size, controls=None, temperature=1.0, verbose=False):
