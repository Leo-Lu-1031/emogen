...
class NoteSeq:
    ...
    @staticmethod
    def process(f): #f is MIDI file
        undupe = lambda l: [i for i,j in zip(l, l[1:])  if i.start<j.start]
        undupe_plus = lambda l: undupe(sorted(l, key = lambda n: 1000*n.start-0.00001*n.pitch))
        f.instruments[0].notes = list(itertools.chain(*[i.notes for i in f.instruments if not i.is_drum]))
        f.instruments[0].notes = undupe_plus(f.instruments[0].notes)
        return f.instruments[0].notes

    @staticmethod
    def from_midi(midi, programs=DEFAULT_LOADING_PROGRAMS):
        notes = NoteSeq.process(midi)
